...
c.build_net(input_tensor=c.input_placeholder, is_training=False, network_type='medium')  

saver = tf.train.Saver(var_list=c.yolo_medium_vars)
saver_all = tf.train.Saver()
session = tf.Session()

session.run(tf.global_variables_initializer())
saver_all.restore(session, '/content/drive/My Drive/DatasetSkripsi/ModelCover500/yolov3')
img_ = cv2.imread('/content/drive/My Drive/ImagesYOLOv3/Proposal.jpeg')
...
bboxes = c.nms(detection_result, 0.5, 0.1) #set the threshold to 0.2, you can modify it
